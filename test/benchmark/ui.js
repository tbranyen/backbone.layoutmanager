...
(function(window, document) {
  ...
  /**
   * Simple function to get a function's body.
   * @param  {Function} fn Function we want to print.
   * @return {String}      String containing the function's body.
   */
  function getFunctionBody(fn) {
    if(!fn) return '';
    var fnString = fn.toString();
    return escapeHtml(
      fnString.substring(fnString.indexOf("{") + 1, fnString.lastIndexOf("}"))
    );
  }
  ...
  ui.on('add', function(event) {
    ...
    tr.innerHTML = [
      ...
      '</th>',
      '<td class="code">',
        '<pre class="brush: js">' + getFunctionBody(bench.fn) + '</pre>',
      '</td>',
    ...
    // Add preparation code
    var prepCode = $('prep-code');
    prepCode.innerHTML = [
      '<div class="setup">',
        '<h3>Setup</h3>',
        '<pre class="brush: js">' + getFunctionBody(bench.setup) + '</pre>',
      '</div>',
      '<div class="teardown">',
        '<h3>Teardown</h3>',
        '<pre class="brush: js">' + getFunctionBody(bench.teardown) + '</pre>',
      '</div>'
    ].join('');
  })
